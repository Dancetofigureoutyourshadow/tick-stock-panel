...
def test_sync_minute_single_accepts_one_year(monkeypatch):
    repo = MagicMock()
    repo.resolve_asset_type.return_value = "stock"
    capset = MagicMock()
    sync = MagicMock(return_value=87_600)
    refresh = MagicMock()
    monkeypatch.setattr(kline_api, "_minute_allowed", lambda _: True)
    monkeypatch.setattr(kline_api.kline_sync, "sync_and_persist_minute", sync)
    monkeypatch.setattr("app.jobs.daily_pipeline._refresh_single_view", refresh)
    ...
    result = asyncio.run(kline_api.sync_minute_single(
        _request(repo, capset),
        {"symbol": "600000.SH", "days": 365},
    ))

    assert result["rows"] == 87_600
    sync.assert_called_once_with(["600000.SH"], repo, capset, days=365, force_full_days=True)
    refresh.assert_called_once_with(repo, "kline_minute")


def test_sync_minute_single_rejects_invalid_days():
    with pytest.raises(HTTPException, match="days 必须在 1 到 365 之间"):
...
def test_sync_minute_single_rejects_days_over_one_year():
    with pytest.raises(HTTPException, match="days 必须在 1 到 365 之间"):
        asyncio.run(kline_api.sync_minute_single(
            _request(),
            {"symbol": "600000.SH", "days": 366},
        ))


@pytest.mark.parametrize("force, skip_existing_days", [(False, True), (True, False)])
def test_sync_minute_single_passes_explicit_range_and_force(monkeypatch, force, skip_existing_days):
    repo = MagicMock()
    repo.resolve_asset_type.return_value = "stock"
    capset = MagicMock()
...
def test_sync_minute_single_can_start_progress_job(monkeypatch):
    repo = MagicMock()
    repo.resolve_asset_type.return_value = "stock"
    capset = MagicMock()
    monkeypatch.setattr(kline_api, "_minute_allowed", lambda _: True)
    ...
    class FakeJobStore:
        def create(self, *, long_running=False):
            assert long_running is True
            return "minute-job-1", True

    monkeypatch.setattr("app.services.pipeline_jobs.job_store", FakeJobStore())
